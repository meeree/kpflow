...
def generate(cfg = DEFAULT_CFG, debug = False, noise = True):
    ...
    # Generate data.
    torch.manual_seed(cfg["seed"])
    A = torch.randn(D, D) / (D**.5)
    W_in = torch.randn(D, D_inp) / (D_inp**.5)
    W_targ = torch.randn(D_targ, D) / (D_targ**.5)

    torch.manual_seed(cfg["seed_data"])
    x = torch.randn((cfg["n_samples"], T, D_inp))
    z = torch.zeros((cfg["n_samples"], D))
    h = []
    for t in range(T):
        z =  z @ A.T + x[:, t] @ W_in.T + (0. if not noise else 0.01 * torch.randn(cfg["n_samples"], D))
        h.append(z.clone().detach())

    h = torch.stack(h, 1)
    target = h.clone()
#    target = h[:, 1:]  # Predict next
    target = target @ W_targ.T
