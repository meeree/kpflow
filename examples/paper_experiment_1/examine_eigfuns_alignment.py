...
if __name__ == '__main__':
    ...
    alignment_all = np.load('rnn_mempro_1.6_to_2/alignment_all.npy')
    g_vals = np.load('rnn_mempro_1.6_to_2/g_vals.npy')
    first_col = fig.add_subplot(gs[:, 0])
    alignment_all, g_vals = alignment_all[115:130], g_vals[115:130] # Subset.
    plt.plot(alignment_all, g_vals)
    plt.title('KA(P P*, P K P*)')
    plt.ylabel('g scale')
    ...
    for idx, (model, hidden) in enumerate(zip(tqdm(models[::1]), hidden_all[::1])):
        ...
        sim = (pf * pphi).mean((1, 3)) / ((pf * pf).mean((1,3)) * (pphi * pphi).mean((1,3)))**0.5
