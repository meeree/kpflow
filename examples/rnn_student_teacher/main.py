...
class Config:
    ...
    train_recurrent_only: bool = True

    # optimization
    batch_size: int = 128
    num_train_steps: int = 9000
    eval_every: int = 100
    lr: float = 1e-2
    momentum: float = 0.0
...
def make_run_name(cfg: Config) -> str:
    run_name = (
        f"g={cfg.g:.3f}, teacher_g={cfg.teacher_g:.3f}, "
...
class TeacherStudentTask:
    """
    Variant 2 setup:
      - sample one shared base recurrent matrix W_base
      - teacher uses W_h = teacher_g * W_base
      - student starts from W_h = g * W_base
      - input/output weights are shared between teacher and student
      - inputs are random rank-input_rank sequences, normalized so total input
        variance is roughly stable across input_rank
    """
    ...
    def __init__(self, cfg: Config):
        ...
        with torch.no_grad():
            ...
            self.shared_W_in_weight = reference.W_in.weight.detach().clone()
            self.shared_W_in_bias = reference.W_in.bias.detach().clone()
        ...
        with torch.no_grad():
            self.teacher.W_in.weight.copy_(self.shared_W_in_weight)
            self.teacher.W_in.bias.copy_(self.shared_W_in_bias)
    ...
    def initialize_student(self, model: VanillaRNN) -> None:
        with torch.no_grad():
            model.W_in.weight.copy_(self.shared_W_in_weight)
            model.W_in.bias.copy_(self.shared_W_in_bias)
        ...
        if self.cfg.train_recurrent_only:
            model.W_in.weight.requires_grad_(False)
            model.W_in.bias.requires_grad_(False)
            model.W_out.weight.requires_grad_(False)
            model.W_out.bias.requires_grad_(False)
            model.W_h.weight.requires_grad_(True)
...
def train_and_save(cfg: Config, save_dir: str = "trained_models"):
    ...
    print("=" * 80)
    print("Shared-base teacher/student tanh RNN task")
    ...
    print(f"train recurrent only: {cfg.train_recurrent_only}")
...
def load_model_and_compare(pt_path, batch_size=16, device=None, plot_example_idx=0):
    ...
    axes[0].set_title(
        f"Shared-base teacher/student RNN | g={cfg.g}, g*={cfg.teacher_g}, input_rank={cfg.input_rank}, seed={cfg.seed}"
    )
    axes[0].legend()
...
def bottom_row_figure():
    ...
    for seed in tqdm(seeds):
        for g in g_values:
            ...
            csv_paths[f"g={g}"] = csv_path
            if os.path.exists(csv_path):
                continue
            train_and_save(cfg, save_dir="trained_models")

    print(csv_paths)
    plot_regime_training_panels(csv_paths)
    plt.show()
...
def plot_ntk_init_heatmaps(
    ...
    log_align=False,
    log_eps=1e-12,
    use_first_order_approx=False, # Approximate NTK space and time rank in a faster way (see paper).
    normalize_align=False,
):
    df = pd.read_csv(csv_path)
    required = {"g", "input_rank", "ntk_space_rank", "ntk_time_rank", "ntk_targ_cos"}
    ...
    Z_space, g_vals, rank_vals = _aggregate_grid(df, "ntk_space_rank", agg=agg)
    Z_time, _, _ = _aggregate_grid(df, "ntk_time_rank", agg=agg)
    Z_full, _, _ = _aggregate_grid(df, "ntk_full_rank", agg=agg)
    Z_align, _, _ = _aggregate_grid(df, "ntk_targ_cos", agg=agg)
    ntk_trace, _, _ = _aggregate_grid(df, "ntk_trace", agg=agg)

    if use_first_order_approx:
        Z_space, _, _ = _aggregate_grid(df, "R_effrank", agg=agg)
        Z_time, _, _ = _aggregate_grid(df, "V_effrank", agg=agg)

    if normalize_align:
        Z_align = Z_align / ntk_trace
    ...
    Z_space_plot, cbar_space = _maybe_log(Z_space, log_space, "ntk_space_rank")
    Z_time_plot, cbar_time = _maybe_log(Z_time, log_time, "ntk_time_rank")
    Z_align_plot, cbar_align = _maybe_log(Z_align, log_align, "ntk_targ_cos")
    ntk_trace_plot, cbar_trace = _maybe_log(ntk_trace, True, "ntk_trace")

    fig, axes = plt.subplots(1, 4, figsize=figsize)
    extent = [g_vals.min(), g_vals.max(), rank_vals.min(), rank_vals.max()]
    G, R = np.meshgrid(g_vals, rank_vals)

    def _draw_panel(ax, Z_plot, Z_raw, cmap, title, xlabel, ylabel=None, cbar_label=None, vmax = None):
        im = ax.imshow(
            Z_plot,
            origin="lower",
            aspect="auto",
    ...
    _draw_panel(
        ...
        Z_space,
        cmap_space,
        r"NTK$_S$ Spatial Rank",
        xlabel=r"Initial Connectivity Scale $g$",
        ylabel=r"Input Rank $r_x$",
        cbar_label=cbar_space,
    )
    ...
    _draw_panel(
        ...
        Z_time,
        cmap_time,
        r"NTK$_S$ Temporal Rank",
        xlabel=r"Initial Connectivity Scale $g$",
        cbar_label=cbar_time,
    )
    ...
    _draw_panel(
        ...
        Z_align,
        cmap_align,
        ('Log ' if log_align else '') + r"Target-NTK Alignment",
        xlabel=r"Initial Connectivity Scale $g$",
        cbar_label=cbar_align,
    )
    ...
    _draw_panel(
        ...
        r"Log NTK Trace",
        xlabel=r"Initial Connectivity Scale $g$",
        cbar_label=cbar_trace,
    )

    plt.figure()
    plt.plot(g_vals, Z_full[-1, :])
    plt.plot(g_vals, Z_space[-1, :])
    plt.plot(g_vals, Z_time[-1, :])
    plt.xlabel('g')
    plt.ylabel('Rank')
    plt.legend(['Full', 'Space', 'Time'])
    ...
    return fig, axes, {
        ...
        "Z_time": Z_time,
        "Z_align": Z_align,
        "Z_space_plot": Z_space_plot,
        "Z_time_plot": Z_time_plot,
        "Z_align_plot": Z_align_plot,
    }
    ...
    for seed in seeds:
        for input_rank in input_ranks:
            for g in g_values:
                ...
                print(
                    f"[seed={seed:02d} | g={g:.4f} | input_rank={input_rank:3d}] "
                    + " | ".join(
                        f"{k}={v:.4g}"
...
def sweep():
    cfg = base_cfg()
    g_values = np.linspace(0.0, 3.0, 50).tolist()
    input_ranks = np.arange(2, 33, 10)[:1]
...
#        g_max=2.6,
        ...
        log_space=False,
        log_time=False,
        log_align=True,   # set True only if your alignment still spans orders of magnitude
