...
class SimpleAttnMLPConfig:
    ...
    positional_n_components: Optional[int] = None
...
class TruncatedSinusoidalPositionalEncoding(nn.Module):
    """
    Standard Vaswani-style sinusoidal positional encoding, with the option
    to keep only the first ``n_components`` channels active and zero the rest.
    This lets us sweep positional-feature count in a way analogous to the
    Fourier-feature sweep, while keeping the addition-style transformer setup.
    """
    def __init__(self, d_model: int, max_seq_len: int, n_components: Optional[int] = None):
        super().__init__()
        self.d_model = d_model
    ...
    def forward(self, T: int) -> torch.Tensor:
        ...
        if self.n_components is not None:
            n_keep = max(0, min(int(self.n_components), self.d_model))
            if n_keep < self.d_model:
                pe[..., n_keep:] = 0.0
        return pe
...
class SingleBlockAttnMLP(nn.Module):
    ...
    def __init__(self, cfg: SimpleAttnMLPConfig):
        ...
        if cfg.use_positional_embedding:
            self.pos_embedding = TruncatedSinusoidalPositionalEncoding(
                d_model=cfg.d_model,
                max_seq_len=cfg.max_seq_len,
                n_components=cfg.positional_n_components,
            )
        else:
...
def panel_positional_encoding_sweep(cfg):
    """
    Vaswani-style sinusoidal positional encoding sweep, analogous to the
    Fourier-feature sweep in panel_D. We vary the number of active positional
    channels while keeping the input dimension fixed at 1.
    """
    ...
    os.makedirs("data", exist_ok=True)
    plt.figure()
    n_components = np.unique(np.linspace(2, cfg.d_model, min(cfg.d_model // 2, 32)).astype(int))
    if n_components[0] % 2 == 1:
        n_components[0] += 1
    n_components = np.unique(2 * np.maximum(1, n_components // 2))

    colors = plt.rcParams['axes.prop_cycle'].by_key()['color']
    for color, B in zip(colors, [5, 10, 15]):
        ranks_ntk_time = []
        if os.path.exists(f'data/pe_ranks_ntk_time_{B}.npy') and os.path.exists('data/pe_n_components.npy'):
            cached_components = np.load('data/pe_n_components.npy')
            if np.array_equal(cached_components, n_components):
                ranks_ntk_time = np.load(f'data/pe_ranks_ntk_time_{B}.npy')

        if len(ranks_ntk_time) == 0:
            for n_comp in tqdm(n_components, desc=f'PE sweep B={B}'):
                cfg.positional_n_components = int(n_comp)
                model = SingleBlockAttnMLP(cfg)

                T = 50
                x = torch.randn(B, T, cfg.d_input)
                ntk_time = get_hidden_ntk_time(model, x)
                ranks_ntk_time.append(ntk_time.effrank(nsamp=100, grammian=False))

            np.save('data/pe_n_components.npy', np.array(n_components))
            np.save(f'data/pe_ranks_ntk_time_{B}.npy', np.array(ranks_ntk_time))

        plt.plot(n_components, ranks_ntk_time, label=f'$n_x = {B}$', color=color)

    plt.xlabel('Positional Encoding Dimension')
    plt.ylabel('Temporal NTK Effective Rank')
    plt.title('Vaswani Positional Encoding Increases Temporal NTK Rank')
    plt.legend(ncol=3)
    plt.savefig('panel_positional_encoding_sweep.pdf')
    plt.show()


def get_hidden_ntk_time(model, x):
    class GetWeightSites(nn.Module):
        def __init__(self, net):
            super().__init__()
            self.net = net
        ...
        def forward(self, x):
            return self.net(x)[1]["V"]

    from kpflow.grad_op import HiddenNTKOperator as NTK
    model_wrap = GetWeightSites(model)
    vs = model_wrap(x)
    ntk = NTK(model_wrap, x, vs, dev=x.device)
    return ntk.partial_avg(-1)



def compute_hidden_ntk_ranks(cfg, B=10, T=50):
    """
    Compute temporal, spatial, and overall effective ranks for the hidden NTK
    of the current single-block attention toy.
    """
    model = SingleBlockAttnMLP(cfg)
    x = torch.randn(B, T, cfg.d_input)
    ...
    ntk_time = ntk.partial_avg(-1)
    ntk_space = ntk.partial_avg(tuple([i for i in range(len(vs.shape) - 1)]))

    rank_time = ntk_time.effrank(nsamp=100, grammian=False)
    rank_space = ntk_space.effrank(nsamp=100, grammian=False)
    rank_full = ntk.effrank(nsamp=100, grammian=False)
    return rank_time, rank_space, rank_full


def panel_attn_input_2d(cfg, B=10, d_inputs=None, d_models=None):
    """
    2D sweep over input dimension and attention/model dimension.

    Produces three heatmaps:
      1. temporal NTK effective rank
      2. spatial NTK effective rank
      3. overall NTK effective rank

    The head count is kept fixed in cfg.n_heads while d_model is varied.
    """
    os.makedirs('data', exist_ok=True)

    if d_inputs is None:
        d_inputs = np.unique(np.linspace(1, 256, 24).astype(int))
    else:
        d_inputs = np.array(d_inputs, dtype=int)

    if d_models is None:
        d_models = np.array([8, 12, 16, 24, 32, 48, 64, 96, 128], dtype=int)
    else:
        d_models = np.array(d_models, dtype=int)

    cfg.use_positional_embedding = False
    cfg.use_fourier_features = False

    cache_base = f'data/panel_attn_input_2d_B{B}_H{cfg.n_heads}'
    path_time = f'{cache_base}_time.npy'
    path_space = f'{cache_base}_space.npy'
    path_full = f'{cache_base}_full.npy'
    path_din = f'{cache_base}_dinputs.npy'
    path_dmodel = f'{cache_base}_dmodels.npy'

    recompute = True
    if all(os.path.exists(p) for p in [path_time, path_space, path_full, path_din, path_dmodel]):
        cached_din = np.load(path_din)
        cached_dmodel = np.load(path_dmodel)
        if np.array_equal(cached_din, d_inputs) and np.array_equal(cached_dmodel, d_models):
            rank_time = np.load(path_time)
            rank_space = np.load(path_space)
            rank_full = np.load(path_full)
            recompute = False

    if recompute:
        rank_time = np.zeros((len(d_models), len(d_inputs)), dtype=float)
        rank_space = np.zeros((len(d_models), len(d_inputs)), dtype=float)
        rank_full = np.zeros((len(d_models), len(d_inputs)), dtype=float)

        for i, d_model in enumerate(tqdm(d_models, desc='attention dim sweep')):
            for j, d_input in enumerate(d_inputs):
                cfg.d_model = int(d_model)
                cfg.d_input = int(d_input)
                rt, rs, rf = compute_hidden_ntk_ranks(cfg, B=B, T=50)
                rank_time[i, j] = rt
                rank_space[i, j] = rs
                rank_full[i, j] = rf

        np.save(path_time, rank_time)
        np.save(path_space, rank_space)
        np.save(path_full, rank_full)
        np.save(path_din, d_inputs)
        np.save(path_dmodel, d_models)

    fig, axes = plt.subplots(1, 3, figsize=(14.5, 4.0), constrained_layout=True)
    panels = [
        (rank_time, 'Temporal NTK Rank'),
        (rank_space, 'Spatial NTK Rank'),
        (rank_full, 'Overall NTK Rank'),
    ]

    extent = [d_inputs[0], d_inputs[-1], d_models[0], d_models[-1]]
    for ax, (arr, title) in zip(axes, panels):
        im = ax.imshow(arr, aspect='auto', origin='lower', extent=extent)
        ax.set_title(title)
        ax.set_xlabel('Input Dim')
        ax.set_ylabel('Attention Dim')
        cbar = plt.colorbar(im, ax=ax)
        cbar.set_label('Effective Rank')

    plt.suptitle(f'2D sweep: input dim vs attention dim ($n_x={B}$, heads={cfg.n_heads}$)$')
    plt.savefig(f'panel_attn_input_2d_B{B}_H{cfg.n_heads}.pdf')

def panel_multihead_input_bottleneck(cfg, Bs=(5, 10, 15), head_list=(1, 2, 4, 8)):
    """
    Compare temporal NTK rank vs input dimension for several head counts.

    This is the appendix version of the input-bottleneck plot: same toy,
    no positional embedding and no Fourier features, but now we vary n_heads.

    Caveat: multi-head attention cannot completely remove the bottleneck induced
    by the input representation x -> h0, but it can create multiple distinct
    temporal mixing channels after h0 and thereby soften the collapse.
    """
    os.makedirs("data", exist_ok=True)
    d_inputs = np.unique(np.linspace(1, 512, 28).astype(int))

    fig, axes = plt.subplots(1, len(Bs), figsize=(4.7 * len(Bs), 3.8), sharey=True)
    if len(Bs) == 1:
        axes = [axes]
    ...
    for ax, B in zip(axes, Bs):
        ranks_x = None
        for i, n_heads in enumerate(head_list):
            cache_path = f'data/mh_ranks_ntk_time_B{B}_H{n_heads}.npy'
            if os.path.exists(cache_path):
                ranks_ntk_time = np.load(cache_path)
            else:
                ranks_ntk_time = []
                for d_input in tqdm(d_inputs, desc=f'B={B}, H={n_heads}'):
                    cfg.d_input = int(d_input)
                    ...
                    T = 50
                    x = torch.randn(B, T, cfg.d_input)
                    _, cache = model(x, need_weights=True)
                    ntk_time = get_hidden_ntk_time(model, x)
                ...
                np.save(cache_path, np.array(ranks_ntk_time))

            ax.plot(d_inputs, ranks_ntk_time, color=colors[i % len(colors)], label=f'{n_heads} head' + ('s' if n_heads > 1 else ''))

        # plot raw input effective dimension once per B for comparison
        x_cache_path = f'data/mh_ranks_x_B{B}.npy'
        if os.path.exists(x_cache_path):
            ranks_x = np.load(x_cache_path)
        else:
            ranks_x = []
            for d_input in tqdm(d_inputs, desc=f'input effdim B={B}'):
                cfg.d_input = int(d_input)
                cfg.n_heads = int(head_list[0])
                cfg.use_positional_embedding = False
                cfg.use_fourier_features = False
                model = SingleBlockAttnMLP(cfg)
                T = 50
                x = torch.randn(B, T, cfg.d_input)
                _, cache = model(x, need_weights=False)
                ranks_x.append(effdim(cache['x_in']))
            np.save(x_cache_path, np.array(ranks_x))

        ax.plot(d_inputs, ranks_x, linestyle='dashed', color='k', label='input effdim')
        ax.set_title(f'$n_x = {B}$')
        ax.set_xlabel('Input Dim')
        ax.grid(alpha=0.2)

    axes[0].set_ylabel('Temporal NTK Effective Rank')
    axes[0].legend(ncol=2)
    plt.suptitle('Multi-head attention softens but does not remove the input bottleneck')
    plt.tight_layout()
    plt.savefig('panel_multihead_input_bottleneck.pdf')



def panel_multihead_modes(cfg, B=10, d_inputs=(4, 32, 128), head_list=(1, 4, 8), nmodes=4):
    """
    Show leading temporal NTK modes for several input dimensions and head counts.
    This is the clean appendix companion to the rank sweep above.
    """
    cfg.use_positional_embedding = False
    cfg.use_fourier_features = False

    colors = plt.rcParams['axes.prop_cycle'].by_key()['color']
    fig, axes = plt.subplots(len(head_list), len(d_inputs), figsize=(4.3 * len(d_inputs), 2.8 * len(head_list)), sharex=True)
    if len(head_list) == 1:
        axes = np.array([axes])
    if len(d_inputs) == 1:
        axes = axes[:, None]
    ...
    T = 50
    for row, n_heads in enumerate(head_list):
        for col, d_input in enumerate(d_inputs):
            cfg.d_input = int(d_input)
            cfg.n_heads = int(n_heads)
            model = SingleBlockAttnMLP(cfg)
            x = torch.randn(B, T, cfg.d_input)
            ntk_time = get_hidden_ntk_time(model, x)
            svals, svecs = ntk_time.svd(max(10, nmodes), grammian=False, compute_vecs=True)

            ax = axes[row, col]
            scale = svals**2 / (svals**2).sum()
            for i in range(nmodes):
                ax.plot(scale[i] * svecs[i, :, :, 0].T, color=colors[i % len(colors)])

            if row == 0:
                ax.set_title(f'$d_{{in}}={d_input}$')
            if col == 0:
                ax.set_ylabel(f'{n_heads} head' + ('s' if n_heads > 1 else ''))
            if row == len(head_list) - 1:
                ax.set_xlabel('Time')
            ax.grid(alpha=0.2)

    plt.suptitle('Leading weighted temporal NTK modes across head count')
    plt.tight_layout()
    plt.savefig('panel_multihead_modes.pdf')
...
#    panel_D(cfg)
    ...
    # Appendix: Vaswani-style sinusoidal positional encoding sweep.
    panel_positional_encoding_sweep(cfg)
    plt.show()

    panel_C(cfg)
    panel_B(cfg)

    # Appendix: 2D sweep over input and attention dimension.
    panel_attn_input_2d(cfg, B=10)

    # Appendix: multi-head comparison with the same no-positional/no-FF toy.
    panel_multihead_input_bottleneck(cfg)
    panel_multihead_modes(cfg)
    plt.show()
