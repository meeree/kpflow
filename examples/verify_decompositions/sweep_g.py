...
from kpflow.op_common import Operator, Projector
...
if __name__ == '__main__':
    ...
    if args.task_str == 'low_rank_forecast':
        task = CustomTaskWrapper('low_rank_forecast', 20, use_noise = False, n_samples = 20, T = 30, seed_data = 10, D_inp = 100, D_targ = 100)
    else:
    ...
    for S in [10, 50, 100, 300]:
        ...
        effdim_k_H = []
        theory_dims = []
        for cell_type in [nn.GRU]: #, nn.RNN]:
            effdim_k_BT.append([])
            effdim_k_H.append([])
            for g in tqdm(gs):
...
#                hidden = model(inputs)[1]
                ...
                hidden, adjoint, err = model.analysis_mode(inputs, targets)[:3]
#                projector = Projector(hidden)
#                print(projector.effdim(-1), effdim(hidden, center = False))
#                jisadjsaoid
                ...
                adj = pop.T(err)
                dh = kop(adj)
#                print(kop.effdim((0,1,2), nsamp = 100, ratio = True))
                print(kop.effdims([-1, (0,1)], nsamp = 100, ratio = True))
                print(pop.effdims([-1, (0,1)], nsamp = 100, ratio = True))
                print(effdim(err), '->', effdim(adj), '->', effdim(dh))

                print(np.linalg.norm(adj.mean((0, 1))), np.linalg.norm(adj.var((0,1))))

                plot_trajectories(project(adj)[1], 2, 2, 1)
                plot_trajectories(project(dh)[1], 2, 2, 2)

                plot_trajectories(project(adj)[1][...,3:], 2, 2, 3)
                plot_trajectories(project(dh)[1][...,3:], 2, 2, 4)
                plt.show()

                asoijdisadas

                print(kop.effdims(nsamp = 100))
                print(pop.effdims(nsamp = 100))
#                pop2 = PropagationOperator_DirectForm(cell, inputs, hidden)
#                pop2.set_debug()
#                print(pop.T.compare(pop2.T, nsamp = 100))
                jaoisdjoisajd

                # Verify kernels perfectly agree. 

                randinp = torch.randn((S, *hidden.shape))
                effdim_k_H[-1].append(effdim(kop.batched_call(randinp)))
                continue
